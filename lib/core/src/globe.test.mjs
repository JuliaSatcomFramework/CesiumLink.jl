// Runnable check for the graticule's geometry: how a line is cut and what it is labelled.
// Run: node lib/core/src/globe.test.mjs
//
// The two passes are pure, but the module they live in draws through Cesium, so the module is
  ...
  plugins: [{
    ...
    setup(build) {
      ...
      build.onLoad({ filter: /.*/, namespace: "stand-in" }, () => ({
        contents: [
          "ArcType", "Cartesian2", "Cartesian3", "ColorGeometryInstanceAttribute", "GeometryInstance",
...
const { cuts, graticuleGeometry, meridianLabel, parallelLabel } = await import(
