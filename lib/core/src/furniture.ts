...
export const FURNITURE_DEFAULTS: Record<FurnitureId, boolean> = {
  ...
  basemap: true,
  // On by default, because both annotation layers are drawn by default and this cell is the only
  // way to take one off from the page. A session that declares every layer off still shows the
  // cell, with its boxes unticked.
  annotations: true,
  navHelp: false,
  inspector: false,
