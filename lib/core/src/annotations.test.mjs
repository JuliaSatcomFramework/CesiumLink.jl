...
// --- a country line keeps its declared width zoomed in, and loses half of it at globe range ---
{
  ...
  // The step is what keeps the rebuild of the lines rare, so every answer sits on it.
  for (const height of [1e6, 4.2e6, 7e6, 1.3e7, 5e7]) {
    assert.equal(borderWidthFactor(height) * 4 % 1, 0,
                 `the factor at ${height} m is rounded to a quarter`);
  }
