  ...
  plugins: [{
    ...
    setup(build) {
      ...
      build.onLoad({ filter: /.*/, namespace: "stand-in" }, () => ({
        contents: [
          "Cartesian3", "GeoJsonDataSource", "HeightReference", "HorizontalOrigin",
          "LabelCollection", "LabelStyle", "SceneMode", "SceneTransforms", "VerticalOrigin",
        ].map((n) => `export const ${n} = {};`).join("\n")
          // The two properties a border line is styled with are built with `new`, so they are
          // classes here and not the empty objects the names above stand in as.
          + "\nexport class ColorMaterialProperty {}\nexport class ConstantProperty {}"
          // The line colours are read as the module loads, so `Color` needs its members.
...
const { annotationBase, behindGlobe, borderWidthFactor, levelAt, visibleNames } = await import(
...
// --- a country line keeps its declared width zoomed in, and loses half of it at globe range ---
{
  assert.equal(borderWidthFactor(2e6), 1, "2,000 km is where the declared width holds");
  assert.equal(borderWidthFactor(3e5), 1, "and every height below it draws the same line");
  assert.equal(borderWidthFactor(2e7), 0.5, "20,000 km is the whole globe, and half the width");
  assert.equal(borderWidthFactor(1e8), 0.5, "further out than that thins no further");
  // The midpoint in the log of the height, which is the geometric mean of the two ends.
  assert.equal(borderWidthFactor(Math.sqrt(2e6 * 2e7)), 0.75, "halfway down in the log is 0.75");
  // The step is what keeps the walk over the entities rare, so every answer sits on it.
  for (const height of [1e6, 4.2e6, 7e6, 1.3e7, 5e7]) {
    assert.equal(borderWidthFactor(height) * 20 % 1, 0,
                 `the factor at ${height} m is rounded to a twentieth`);
  }
  // A camera the scene cannot put over the globe reports no useful height, and a line still needs
  // a width: the widest one, which is what the declaration asked for.
  assert.equal(borderWidthFactor(0), 1, "a height of nothing draws the declared width");
}
