...
const { WsTransport, createHeldDelivery, firstDeclaration, packFrame, splitFrame } =
...
// --- a handler that throws on a held frame loses that frame only ---
{
  const d = createHeldDelivery();
  for (const n of [1, 2, 3]) d.deliver("window", { n }, new Uint8Array(0));
  ...
  d.on("window", (p) => {
    if (p.n === 1) throw new Error("bad frame");
    seen.push(p.n);
  });
  await Promise.resolve();
  console.error = consoleError;

  assert.deepEqual(seen, [2, 3], "the frames held behind the failed one still arrive");
  assert.equal(errors.length, 1, "the failure is reported once");
  assert.match(String(errors[0][0]), /window/, "and the report names the method");
}
