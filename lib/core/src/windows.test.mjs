...
// --- the clock reaches the server before the crossing it happened under ------------------------
{
  // Both callbacks into one list, so the order they were called in is what is asserted.
  const seen = [];
  const h = harness({
    onClock: (multiplier, playing) => seen.push({ clock: { multiplier, playing } }),
    onCrossing: (index) => seen.push({ crossing: index }),
  });

  h.windows.deliver(msg({ startFrame: 0, count: 4, totalFrames: 20, mode: "replace",
                          payloads: { m: {} } }));
  assert.deepEqual(seen, [{ clock: { multiplier: 60, playing: true } }, { crossing: 0 }],
                   "the opening window states the run's clock before the keyframe it crosses into");

  h.tickAt(60);
  assert.deepEqual(seen.at(-1), { crossing: 1 }, "a crossing under an unchanged clock says it once");

  // The user turns playback around, and a control then re-pushes the scene over a new range. The
  // range writes the multiplier itself, so the crossing that follows belongs to the new direction.
  h.clock.multiplier = -60;
  h.tickAt(60);
  assert.deepEqual(seen.at(-1), { clock: { multiplier: -60, playing: true } });
  h.windows.deliver(msg({ startFrame: 0, count: 4, totalFrames: 30, mode: "replace",
                          payloads: { m: {} } }));
  assert.deepEqual(seen.slice(-2), [{ clock: { multiplier: 60, playing: true } }, { crossing: 0 }],
                   "the re-declared run corrects the direction before it reports the crossing");
}
