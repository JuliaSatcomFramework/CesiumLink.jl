...
export class ColorMaterialProperty { constructor(color) { this.color = color; } }
export class ConstantProperty { constructor(value) { this.value = value; } }
...
export const GeoJsonDataSource = {
  load(source, options) {
    ...
    // entities.values is what the caller walks to restyle the lines. Two of them, so a case can
    // say that the style reaches every line and not only the first.
    return Promise.resolve({
      url: source, options, show: true, features: source.features,
      entities: { values: [{ polyline: {} }, { polyline: {} }] },
    });
  },
};
...
// pick that left the borders alone would draw white lines over a pale relief map (ADR-0036).
{
  ...
  const widget = await createScene(makeContainer(), { baseUrl: "cesium/", imagery: set },
                                   makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  const lines = () => widget.dataSources.sources[0].entities.values;
  assert.deepEqual(lines().map((e) => e.polyline.material.color), ["#0000008c", "#0000008c"],
  ...
  assert.deepEqual(lines().map((e) => e.polyline.width.value), [1.5, 1.5], "and half its width");

  const source = widget.dataSources.sources[0];
  annotationsOf(widget).styleBorders({ color: set[1].borderColor, width: set[1].borderWidth });
  assert.deepEqual(lines().map((e) => e.polyline.material.color), ["#4a3520b3", "#4a3520b3"],
                   "a pick restyles the lines that are already there");
  assert.deepEqual(lines().map((e) => e.polyline.width.value), [0.75, 0.75], "width and all");
  assert.equal(widget.dataSources.sources[0], source,
               "the same data source, so the file is not loaded a second time");
  reset();
}
...
// --- the lines thin towards the whole-globe view, on the pass that pages the names ---
{
  const widget = await createScene(makeContainer(),
                                   ...
                                   makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  const widths = () => widget.dataSources.sources[0].entities.values.map((e) => e.polyline.width.value);
...
// --- a colour the browser cannot read draws the default, and says so once ---
{
  ...
  const lines = widget.dataSources.sources[0].entities.values;
  assert.deepEqual(lines.map((e) => e.polyline.material.color), ["#ffffff8c", "#ffffff8c"],
