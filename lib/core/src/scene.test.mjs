...
export const HeightReference = { CLAMP_TO_GROUND: "clamped" };
...
export const GeoJsonDataSource = {
  // The country borders name a URL; the region borders hand over a filtered collection.
  load(source, options) {
...
// The region lines, one per band, so a case can say which band the camera height asked for.
globalThis.__cesium.regions = {
  type: "FeatureCollection",
  features: [
    { type: "Feature", properties: { minz: 1 }, geometry: { type: "LineString", coordinates: [] } },
    { type: "Feature", properties: { minz: 3 }, geometry: { type: "LineString", coordinates: [] } },
  ],
};
globalThis.fetch = (url) => {
  globalThis.__cesium.assets.push(url);
  const body = url.endsWith("region-borders.geojson")
    ? globalThis.__cesium.regions
    : globalThis.__cesium.places;
...
// is not an imagery layer at all cannot be taken off by a switch (ADR-0036).
{
  ...
  assert.ok(globalThis.__cesium.assets.includes("annotations/region-borders.geojson"),
            "and so are the region lines, which a session can switch on later");
...
// Drop the camera to a height and let the layers repaint, the way a zoom does. The second wait is
// for the data source the repaint loads.
async function descend(widget, height) {
  widget.camera.positionCartographic.height = height;
  for (const fn of widget.listeners) fn();
  await new Promise((r) => setTimeout(r, 0));
  await new Promise((r) => setTimeout(r, 0));
}

// --- the region lines are off until a session asks, and then carry the camera's band alone ---
{
  const plain = await createScene(makeContainer(), { baseUrl: "cesium/" }, makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  assert.equal(plain.dataSources.sources.length, 1,
               "a default session draws the country borders and no region line at all");
  reset();

  const drawn = await createScene(makeContainer(),
                                  { baseUrl: "cesium/", regionBorders: true }, makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  assert.equal(drawn.dataSources.sources.length, 1,
               "asking for them still draws none from a whole-globe view");

  await descend(drawn, 1e7);
  assert.equal(drawn.dataSources.sources.length, 2, "a level down adds a second data source");
  assert.deepEqual(drawn.dataSources.sources[1].features.map((f) => f.properties.minz), [1],
                   "and it holds the band this camera height reaches, not the whole file");
  assert.equal(drawn.dataSources.sources[1].options.strokeWidth, 1.5,
               "a region line is thinner than a country line, which is two pixels wide");
  reset();
}

// --- a region line never draws while the country lines are off (ADR-0036) ---
{
  const held = await createScene(makeContainer(),
                                 { baseUrl: "cesium/", regionBorders: true, countryBorders: false },
                                 makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  await descend(held, 1e7);
  assert.equal(held.dataSources.sources.length, 1,
               "the country lines are off, so the region lines the session asked for draw nothing");

  const layers = annotationsOf(held);
  layers.showBorders(true);
  await new Promise((r) => setTimeout(r, 0));
  assert.equal(held.dataSources.sources.length, 2, "the country switch brings them back");
  layers.showRegions(false);
  await new Promise((r) => setTimeout(r, 0));
  assert.equal(held.dataSources.sources.length, 1, "and each layer still switches on its own");
  reset();
}
