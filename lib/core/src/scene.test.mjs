...
// carries `textContent`, the attribute the credit line is marked with, and a way off its parent —
...
// --- the credit follows the pick: one line, rewritten, and taken down by an entry without one ---
{
  ...
  await createScene(container, {
    ...
    imagery: [
      ...
      { url: "https://tile/{z}/{x}/{y}.png", layout: "xyz", key: "osm",
        credit: "\u00a9 OpenStreetMap contributors" },
      { url: "", layout: "xyz", key: "offline_natural_earth", bundled: true },
    ],
  });
  assert.equal(container.children[0].textContent, "NASA EOSDIS GIBS",
  ...
  setCredit(container, "\u00a9 OpenStreetMap contributors");
  assert.equal(container.children.length, 1, "a switch rewrites the line rather than adding one");
  assert.equal(container.children[0].textContent, "\u00a9 OpenStreetMap contributors",
