...
export class CesiumWidget {
  constructor(container, options) {
    ...
    // The canvas the widget draws into, and the page it joins: the builder marks it and reads the
    // marks of the ones already there, so a stand-in that holds neither cannot answer the question.
    const attributes = {};
    this.canvas = {
...
globalThis.__cesium = { fetched: [], built: [], canvases: [] };
// The DOM the builder touches: the detached div the Cesium credits are sent to, the credit line a
// declared basemap adds, and the canvases the page already holds. `textContent` is the only property
// the credit is allowed to write, so the stand-in element carries nothing else.
globalThis.document = {
  createElement: () => ({ style: {} }),
...
// Chrome hands two WebGL canvases of one size the same recycled GPU buffer, and the second viewer
// then flashes the first viewer's picture. Every viewer therefore takes a size slot of its own.
{
  ...
  assert.equal(first.resolutionScale, 1, "the one viewer a page usually holds pays nothing");
  ...
  assert.ok(Math.min(...scales) > 0.99, "and every one of them stays within a percent of what it asked for");

  // A notebook cell that re-runs destroys its viewer and builds another. The slot the destroyed one
  // held must come back, or a cell that is re-run all afternoon walks its own resolution down.
  ...
  assert.equal(
    ...
    "a viewer that is destroyed gives its slot back to the next one built",
  );
  assert.notEqual(replacement.resolutionScale, first.resolutionScale, "and it takes nobody else's");
  reset();
}
