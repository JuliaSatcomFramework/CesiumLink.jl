...
// A stand-in for DOMPurify, as `CESIUM_STUB` is one for Cesium. The real sanitizer parses HTML and
// needs a DOM, and this file runs in Node. The stand-in records every string it is given and takes a
// script element out, so a case can read what the builder does with a credit: hand it over whole,
// and draw the answer and nothing else.
const PURIFY_STUB = `
globalThis.__purify = { seen: [] };
export default {
  sanitize(html) {
...
// credit line a declared basemap adds, and the canvases that the page already holds. An element
// carries `innerHTML`, the attribute the credit line is marked with, and a way off its parent —
// which is what the credit setter takes it down by.
globalThis.document = {
  createElement: () => ({
    style: {},
    attributes: {},
...
const makeContainer = () => ({
  ...
  querySelector(selector) {
    const attribute = selector.match(/^:scope > \[([\w-]+)\]$/)?.[1];
    assert.ok(attribute, `the stand-in answers ":scope > [attribute]" and nothing else, not ${selector}`);
    return this.children.find((c) => c.getAttribute?.(attribute) !== null) ?? null;
  },
});
  ...
  plugins: [
    {
      ...
      setup(build) {
        ...
        build.onLoad({ filter: /.*/, namespace: "stand-in" }, (args) => ({
          contents: args.path === "dompurify" ? PURIFY_STUB : CESIUM_STUB,
          loader: "js",
        }));
      },
    },
...
const { basemapProviders, basemapSet, createScene, loadImagery, setCredit } = await import(
...
{
  const widget = await createScene(makeContainer(), { baseUrl: "cesium/" });
...
{
  const widget = await createScene(makeContainer(), { baseUrl: "cesium/", ellipsoid: ODD });
  assert.deepEqual(
...
// --- a credit is HTML: the sanitizer sees it whole, and its answer is the whole of what is drawn ---
{
  ...
  assert.doesNotMatch(el.style.cssText, /pointer-events/,
    "the line takes a click, or the anchor in it could not be followed");

  // A plain string is HTML that holds no element, so it draws as itself.
  setCredit(container, "Somebody else's tiles");
  assert.equal(container.children[0].innerHTML, "Somebody else's tiles",
    "a credit with no markup in it draws as that string");
  reset();
}
...
// --- the credit follows the pick: one line, rewritten, and taken down by an entry without one ---
{
  ...
  assert.equal(container.children[0].innerHTML, "NASA EOSDIS GIBS",
  ...
  setCredit(container, "Somebody else's tiles");
  assert.equal(container.children.length, 1, "a switch rewrites the line rather than adding one");
  assert.equal(container.children[0].innerHTML, "Somebody else's tiles",
  ...
  setCredit(container, undefined);
  assert.deepEqual(container.children, [],
    "an entry that declares no credit takes the line down");
  reset();
}
...
// --- a set of more than one: entry 0 is what the globe wears at startup ---
{
  ...
  assert.equal(container.children[0].innerHTML, "The first source", "and entry 0 owns the credit");
  reset();
}
