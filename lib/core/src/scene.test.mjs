...
export class Rectangle {
  constructor(west, south, east, north) { Object.assign(this, { west, south, east, north }); }
  static contains(rectangle, position) {
    return position.longitude >= rectangle.west && position.longitude <= rectangle.east &&
      position.latitude >= rectangle.south && position.latitude <= rectangle.north;
  }
}
Rectangle.MAX_VALUE = new Rectangle(-Math.PI, -Math.PI / 2, Math.PI, Math.PI / 2);
export const Color = {
  DIMGRAY: "the flat colour of a bare globe",
  BLACK: { withAlpha: (a) => "black/" + a },
  WHITE: { withAlpha: (a) => "white/" + a },
  TRANSPARENT: "transparent",
  // Cesium answers \`undefined\` for a string it cannot read, and says nothing about it. A stand-in
  // that answered the string back would hide the fallback the viewer has to make.
  fromCssColorString: (css) => (/^#[0-9a-f]{3,8}$/i.test(css) ? css : undefined),
};
export class Cartesian3 {
  static fromDegrees(lon, lat) { return { lon, lat }; }
  static fromDegreesArray(degrees) { return degrees; }
}
...
// The parts a border primitive is built from. Each keeps the options it was built with, so a case
// can read the width and the colour of every line back.
export const ArcType = { RHUMB: "rhumb" };
export const ColorGeometryInstanceAttribute = { fromColor: (color) => color, toValue: (color) => color };
export class GeometryInstance { constructor(options) { Object.assign(this, options); } }
export class PolylineGeometry { constructor(options) { Object.assign(this, options); } }
export class PolylineColorAppearance {}
PolylineColorAppearance.VERTEX_FORMAT = "position and colour";
// No frame is ever drawn here, so a primitive is ready only when a case says so. Before that, a
// per-instance attribute is out of reach, as it is in Cesium before the first update.
export class Primitive {
  constructor(options) { Object.assign(this, options); this.ready = false; }
  getGeometryInstanceAttributes(id) {
    if (!this.ready) throw new Error("must call update before calling getGeometryInstanceAttributes");
    return this.geometryInstances.find((instance) => instance.id === id).attributes;
  }
}
export class CesiumWidget {
  constructor(container, options) {
    ...
    this.clock = {};
    // The annotation layers: a primitive collection, plus the camera whose movement repopulates
    // the names.
    ...
    this.scene.primitives = {
      add: (p) => (this.primitives.push(p), p),
      remove: (p) => {
        this.removed.push(p);
        return this.primitives.splice(this.primitives.indexOf(p), 1).length === 1;
      },
    };
    // The stand-in renders one frame the moment a listener is added: the throwaway primitive that
    // warms the worker pool comes off in it, and every asynchronous build finishes in it.
    this.scene.postRender = {
      addEventListener: (fn) => {
        for (const p of this.primitives) if (p.asynchronous) p.ready = true;
        fn();
      },
      removeEventListener() {},
    };
...
// The country borders: one line of each kind the file holds, and one member too short to be a
// line. Two lines come out of it, so a case can say that a style reaches every line and not only
// the first.
globalThis.__cesium.borders = {
  type: "FeatureCollection",
  features: [
    { geometry: { type: "LineString", coordinates: [[0, 0], [1, 1]] } },
    { geometry: { type: "MultiLineString", coordinates: [[[2, 2], [3, 3]], [[4, 4]]] } },
  ],
};
globalThis.fetch = (url) => {
  globalThis.__cesium.assets.push(url);
  const body = url.endsWith(".geojson") ? globalThis.__cesium.borders : globalThis.__cesium.places;
...
// --- no ellipsoid named: WGS84 stands, and the widget is told nothing ---
{
  ...
  assert.equal(Ellipsoid.default, Ellipsoid.WGS84, "the build writes WGS84 into the global default");
}
...
// --- a scene with no ellipsoid, built after one on another body, is back on WGS84 ---
{
  await createScene(makeContainer(), { baseUrl: "cesium/", ellipsoid: ODD }, makeOverlay());
  const widget = await createScene(makeContainer(), { baseUrl: "cesium/" }, makeOverlay());
  assert.equal(built().defaultAtBuild, Ellipsoid.WGS84,
               "the static is WGS84 again before the second widget is constructed");
  assert.equal(widget.scene.ellipsoid, Ellipsoid.WGS84, "so the second globe is not the first body");
}
...
// --- tiling: "gibs-geographic" states the NASA grid, and its own tile size with it ---
{
  const widget = await createScene(makeContainer(), {
    baseUrl: "cesium/",
    ellipsoid: ODD,
    imagery: { url: "https://gibs.test/{z}/{y}/{x}.jpeg", layout: "xyz",
               tiling: "gibs-geographic", maxLevel: 11 },
  }, makeOverlay());
  const { tilingScheme, tileWidth, tileHeight, maximumLevel } =
    built().options.baseLayer.provider.options;
  assert.equal(tilingScheme.constructor.name, "GibsGeographicTilingScheme", "the declared scheme");
  assert.equal(tilingScheme.ellipsoid, widget.scene.ellipsoid, "on the declared shape as well");
  // A GIBS tile is 512 pixels square. Cesium's own default is 256, and a provider given the wrong
  // size asks for the right tiles and draws them at half scale.
  assert.deepEqual([tileWidth, tileHeight, maximumLevel], [512, 512, 11], "the GIBS tile size");
}
...
// The country borders on a widget: the primitives built from geometry instances, of which there is
// one at any time. The names are the other primitive, a label collection.
const bordersOf = (widget) => widget.primitives.filter((p) => p.geometryInstances);
const borderLines = (widget) => {
  const borders = bordersOf(widget);
  assert.equal(borders.length, 1, "the borders are one primitive, never two");
  return borders[0].geometryInstances;
};
...
// is not an imagery layer at all cannot be taken off by a switch (ADR-0036).
{
  ...
  assert.equal(widget.primitives.length, 2, "the names and the boundaries are two primitives");
  // The worker pool is warmed by one asynchronous primitive that comes off after a frame, so the
  // scene keeps no trace of it.
  assert.equal(widget.removed.length, 1, "one throwaway primitive warmed the worker pool");
  assert.equal(widget.removed[0].asynchronous, true, "which is what starts the pool");
  assert.equal(widget.primitives[0].length, 1,
               "and the collection holds only what this camera height carries");
  const [borders] = bordersOf(widget);
  assert.equal(borders.asynchronous, false, "the boundaries are built with no geometry worker");
  assert.equal(borderLines(widget).length, 2, "one instance per line, and a lone point is no line");
  assert.deepEqual(borderLines(widget).map((line) => line.geometry.positions),
                   [[0, 0, 1, 1], [2, 2, 3, 3]], "each with its own positions");
  assert.deepEqual(borderLines(widget).map((line) => line.geometry.arcType), ["rhumb", "rhumb"],
                   "along rhumb lines, as a GeoJSON line is drawn");
...
// --- the two flags, each taking one layer off and leaving the other drawn (ADR-0036) ---
{
  ...
  assert.equal(bordersOf(off)[0].show, true, "and the borders are still drawn");

  const bare = await createScene(makeContainer(),
                                 { baseUrl: "cesium/", countryBorders: false }, makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  assert.equal(bordersOf(bare)[0].show, false, "the borders come off on their own");
...
// --- each layer still switches on its own once the globe is built (ADR-0036) ---
{
  ...
  assert.equal(bordersOf(held)[0].show, false, "the borders start off, as declared");
  layers.showBorders(true);
  assert.equal(bordersOf(held)[0].show, true, "and the switch brings them back");
  layers.showPlaces(false);
  assert.equal(held.primitives[0].length, 0, "while the names come off on their own");
}

// --- the country borders wear the style of the basemap on the globe, and follow the pick ---
// The picker's creation function states the style of the entry the reader landed on, which is what
// `styleBorders` is called with here. The right colour depends on what lies under the line, so a
// pick that left the borders alone would draw white lines over a pale relief map (ADR-0036).
{
  const set = [
    { url: "a/{z}/{x}/{y}.png", layout: "xyz", borderColor: "#0000008c", borderWidth: 3 },
    { url: "b/{z}/{x}/{y}.png", layout: "xyz", borderColor: "#4a3520b3", borderWidth: 1.5 },
  ];
  const widget = await createScene(makeContainer(), { baseUrl: "cesium/", imagery: set },
                                   makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  const colours = () => borderLines(widget).map((line) => line.attributes.color);
  const widths = () => borderLines(widget).map((line) => line.geometry.width);
  assert.deepEqual(colours(), ["#0000008c", "#0000008c"],
                   "the globe opens on entry 0, and every line wears the colour it asks for");
  // The stand-in camera sits at 20,000 km, which is the whole globe, so a line draws at half the
  // width its basemap declares.
  assert.deepEqual(widths(), [1.5, 1.5], "and half its width");

  // A frame was drawn, so the lines' colour attributes are in reach.
  const before = bordersOf(widget)[0];
  before.ready = true;
  const fetched = globalThis.__cesium.assets.length;
  annotationsOf(widget).styleBorders({ color: set[1].borderColor, width: set[0].borderWidth });
  assert.deepEqual(colours(), ["#4a3520b3", "#4a3520b3"],
                   "a pick that alters only the colour writes it onto the lines that are there");
  assert.equal(bordersOf(widget)[0], before, "the same primitive, not a build");

  annotationsOf(widget).styleBorders({ color: set[1].borderColor, width: set[1].borderWidth });
  assert.deepEqual(widths(), [0.75, 0.75], "a pick that alters the width builds the lines again");
  assert.deepEqual(colours(), ["#4a3520b3", "#4a3520b3"], "in the colour asked for");
  assert.notEqual(bordersOf(widget)[0], before, "as a new primitive");
  assert.equal(globalThis.__cesium.assets.length, fetched, "from the file already parsed");
}

// --- a colour that arrives before the first frame is drawn goes in through a build ---
// The attribute a colour is written into exists only once the primitive was updated, which is
// something a widget with no frame drawn yet cannot say for its lines.
{
  const widget = await createScene(makeContainer(), { baseUrl: "cesium/" }, makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  const before = bordersOf(widget)[0];
  assert.equal(before.ready, false, "no frame was drawn");
  annotationsOf(widget).styleBorders({ color: "#4a3520b3" });
  assert.deepEqual(borderLines(widget).map((line) => line.attributes.color),
                   ["#4a3520b3", "#4a3520b3"], "and the lines still wear the colour");
  assert.notEqual(bordersOf(widget)[0], before, "as a new primitive, which costs nothing then");
}

// --- the lines thin towards the whole-globe view, on the pass that pages the names ---
{
  const widget = await createScene(makeContainer(),
                                   { baseUrl: "cesium/", imagery: { url: "a/", layout: "tms" } },
                                   makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  const widths = () => borderLines(widget).map((line) => line.geometry.width);
  assert.deepEqual(widths(), [1, 1], "at 20,000 km a two-pixel line is drawn at one");
  // Flying in: the same two events the names are repaged on.
  const before = bordersOf(widget)[0];
  widget.camera.positionCartographic.height = 1e6;
  for (const fire of widget.listeners) fire();
  assert.deepEqual(widths(), [2, 2], "and below 2,000 km it is drawn at the width declared");
  assert.notEqual(bordersOf(widget)[0], before, "which is a build, the width being geometry");
  // A camera report that leaves the rounded factor where it was builds nothing.
  const built = bordersOf(widget)[0];
  widget.camera.positionCartographic.height = 5e5;
  for (const fire of widget.listeners) fire();
  assert.equal(bordersOf(widget)[0], built, "and one that alters no width builds nothing");
}

// --- a colour the browser cannot read draws the default, and says so once ---
{
  const said = [];
  ...
  const widget = await createScene(makeContainer(), { baseUrl: "cesium/" }, makeOverlay());
  await new Promise((r) => setTimeout(r, 0));
  const layers = annotationsOf(widget);
  layers.styleBorders({ color: "chartreuse-ish" });
  layers.styleBorders({ color: "chartreuse-ish" });
  console.warn = warn;
  assert.equal(said.length, 1, "one line for the fault, not one per call");
  assert.match(said[0], /chartreuse-ish/, "and it names the string that could not be read");
  assert.deepEqual(borderLines(widget).map((line) => line.attributes.color),
                   ["#ffffff8c", "#ffffff8c"],
                   "the lines are drawn in the default colour rather than in none at all");
}
