...
export const GeoJsonDataSource = {
  ...
  load(source, options) {
    ...
    return Promise.resolve({ url: source, options, show: true, features: source.features });
  },
};
...
// --- the region lines are off until a session asks, and then carry the camera's band alone ---
{
  ...
  await new Promise((r) => setTimeout(r, 0));
  assert.equal(drawn.dataSources.sources.length, 2, "asking for them adds a second data source");
  ...
  assert.equal(drawn.dataSources.sources[1].options.strokeWidth, 1,
