...
export class CesiumWidget {
  constructor(container, options) {
    ...
    // The stand-in renders one frame the moment a listener is added, and that frame finishes every
    // asynchronous build, so the throwaway primitive that warms the worker pool goes at once.
...
// is not an imagery layer at all cannot be taken off by a switch (ADR-0036).
{
  ...
  // The worker pool is warmed by one asynchronous primitive that goes as soon as it is built, so
  // the scene keeps no trace of it.
