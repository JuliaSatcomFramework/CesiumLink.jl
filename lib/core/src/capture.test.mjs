...
/** A `data:` URL over these bytes, the way a canvas writes one. */
const dataUrl = (bytes) => "data:image/png;base64," + Buffer.from(bytes).toString("base64");

/** A picture the way a canvas gives one: the PNG signature, then bytes that stand for pixels. */
const PNG = [137, 80, 78, 71, 13, 10, 26, 10, 1, 2, 3];

// --- the bytes of a data URL are the bytes the canvas wrote ---
{
  assert.deepEqual([...bytesOfDataUrl(dataUrl(PNG))], PNG);
}

/**
 * A widget that answers what a capture reads, and records what the capture did to it.
 *
 * `hasGl` is whether the scene gives its graphics context up at all, `max` is the texture limit
 * that context states, and `lost` is a context that lost the GPU under it.
 */
function fakeWidget({ max = 0, width = 800, height = 600, lost = false,
                      hasGl = max > 0 || lost } = {}) {
  const gl = { MAX_TEXTURE_SIZE: 1, getParameter: () => max, isContextLost: () => lost };
  const widget = {
    resolutionScale: 0.87,
    resizes: 0,
    renders: [],
    canvas: { width, height, toDataURL: () => dataUrl(PNG) },
    scene: {
      context: hasGl ? { _gl: gl } : undefined,
...
// --- a canvas that encodes nothing is a refusal, and never a picture of zero bytes ---
{
  const widget = fakeWidget();
  // `data:,` is what a canvas answers when it is past the area one canvas may hold. A browser caps
  // that area far below the GPU texture limit, and it reports the cap as neither a throw nor a
  // failed render.
  widget.canvas.toDataURL = () => "data:,";
  const shot = takeCapture(widget, 4);
  assert.equal(shot.ok, false);
  assert.match(shot.error, /3200x2400/, "the reason names the size the call asked for");
  assert.equal(widget.resolutionScale, 0.87, "a refused capture still puts the buffer back");
}

// --- bytes that carry no PNG signature are a refusal too, whatever made them ---
{
  const widget = fakeWidget();
  widget.canvas.toDataURL = () => dataUrl([0, 0, 0, 0, 0, 0, 0, 0, 1, 2, 3]);
  assert.equal(takeCapture(widget, 1).ok, false, "the signature is read, not only the length");
}

// --- a lost graphics context refuses before the render, rather than drawing a transparent PNG ---
{
  const widget = fakeWidget({ lost: true });
  const shot = takeCapture(widget, 1);
  assert.equal(shot.ok, false);
  assert.match(shot.error, /context is lost/);
  assert.deepEqual(widget.renders, [], "the refusal comes before the render");
}

// --- a context that states no texture size refuses, rather than turning the size check off ---
{
  const widget = fakeWidget({ max: 0, hasGl: true });
  const shot = takeCapture(widget, 1);
  assert.equal(shot.ok, false);
  assert.match(shot.error, /no texture size/,
    "a 0 from a live context is an answer nobody can build a limit on");
  assert.deepEqual(widget.renders, []);
}
...
// --- a long press that sends no click leaves the next tap working ---
{
  const widget = fakeWidget();
  const cell = makeEl("div");
  const item = captureCell(cell, widget);
  ...
  const button = root.children[0];
  const panel = root.children[root.children.length - 1];
  // A finger that slides off the button sends the click to an ancestor, and Android raises its own
  // context menu and then drops the click. So the press that opened the popup may never reach the
  // button, and the flag it set must not outlive it.
  button.fire("pointerdown", { pointerType: "touch" });
  await new Promise((done) => setTimeout(done, 700));
  assert.equal(panel.style.display, "flex", "the long press opened the popup");
  button.fire("pointerup", {});
  // The next tap is a real one, and it must act.
  button.fire("pointerdown", { pointerType: "touch" });
  button.fire("pointerup", {});
  button.fire("click", {});
  assert.equal(panel.style.display, "none", "the tap after a click-less long press closes the popup");
  item.destroy();
}
