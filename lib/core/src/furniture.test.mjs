...
// --- the annotations cell holds one box per layer, and states the region layer's dependency ---
//
...
// can catch. So read the cell out of the source that owns it.
{
  const ui = await readFile(new URL("./clock-ui.ts", import.meta.url), "utf8");
  const cell = /function annotationsCell\([\s\S]*?\n}/.exec(ui)[0];
