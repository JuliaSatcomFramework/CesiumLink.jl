...
// --- the defaults are stated once, and four items are off ---
{
  ...
  assert.equal(FURNITURE_DEFAULTS.annotations, true,
    "two of the three annotation layers are drawn by default, so the cell is drawn too");
}

// --- the annotation rows hold one box per layer, and state the region layer's dependency ---
//
// A region line never draws while the country lines are off (ADR-0036). The box that asks for one
// therefore goes dead while the country box is clear, and neither half of that is something a type
// can catch. So read the rows out of the source that owns them, and check that both homes take
// them: the picker's drop-down, and the cell a globe without a picker falls back to.
{
  ...
  assert.deepEqual([...cell.matchAll(/row\("([^"]+)"/g)].map((m) => m[1]),
    ["Place names", "Country borders", "Region borders"],
    "one box per layer, in the order the layers stack");
  assert.match(cell, /regions\.disabled = !layers\.borders/,
    "the region box starts dead when the session declared no country borders");
  assert.match(cell, /regions\.disabled = !draw/,
    "and the country box drives it, so the reader sees the dependency");
}
