...
//
// @cesium/engine 26.2.0 publishes `Build/Workers` uncleaned between two release builds: 172 of its
// 282 files, 6.1 MB, are named by nothing. Copying the directory whole carries all of them into the
// release artifact, which ships as a Julia artifact and is downloaded.
//
// The roots are the files not named `chunk-*.js`. Cesium asks for a worker by name at run time,
// through `TaskProcessor`, so an entry point is a run-time root that no import graph points at —
// which is also why a bundler cannot do this and the tree has to be walked here.
//
...
//
// A clean release leaves this with nothing to do.
{
  const dir = join(dist, "cesium/Workers");
  const present = new Set(readdirSync(dir));
  const chunkName = /chunk-[^\s"'`;,)]+\.js/g;
