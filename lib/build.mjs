...
// The Slate host's mount point. Slate injects this module's text into the notebook page, so it is
// copied rather than bundled; the Julia extension reads it from here, which is what keeps it and the
// bundle it imports the same build.
...
// extension's channel, the Slate host over a notebook's own page socket. They are separate entries
// rather than one with a flag, so a page carries only the transport it uses.
//
// One build, because esbuild shares a chunk only between the entry points of a single build: one
// build per host compiles the whole of Cesium once per host and saves nothing. The object form of
