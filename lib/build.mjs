...
cpSync(join(root, "browser/index.html"), join(dist, "index.html"));
cpSync(join(root, "player/index.html"), join(dist, "player.html"));
...
const loader = { ".css": "css", ".png": "file", ".jpg": "file", ".gif": "file", ".svg": "file" };

// The three hosts, in ONE build. Each is the same Core reached through a different transport: the
// browser host over a socket, the player from a recording file, the VSCode host through the
// extension's channel. They are separate entries rather than one with a flag, so a page carries
// only the transport it uses.
//
// One build, because esbuild shares a chunk only between the entry points of a single build:
// `splitting` across three builds compiles the whole of Cesium three times over and saves nothing.
// The object form of `entryPoints` names the outputs — `dist/app.js`, `dist/player.js`,
// `dist/vscode.js` and their `.css` siblings — which the four pages and `extension.js` all name.
// An array with `outdir` cannot: all three sources are called `main.ts`.
//
...
const hostOpts = {
  entryPoints: {
    app: join(root, "browser/main.ts"),
    player: join(root, "player/main.ts"),
    vscode: join(root, "vscode/main.ts"),
  },
  bundle: true,
  format: "esm",
  splitting: true,
