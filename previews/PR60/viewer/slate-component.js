...
// The Slate presenter window is a second page of the notebook, at `?present=1`. It renders every cell
// but shows only copies of the slide DOM, and a copy of a canvas is blank. A viewer there is a second
// client on the same channel: it sends its own `ready` and its own events up, and the scene in the
// audience page stutters. That window thus gets a note, and no viewer. The note has the height of the
// viewer, so a slide preview there keeps the lay-out that the audience sees.
const PRESENTER = new URLSearchParams(location.search).get("present") === "1";

export default ({ params }) => PRESENTER
  ? html`<div style="height:${params.height ?? 520}px;display:flex;align-items:center;justify-content:center;font-style:italic;border:1px dashed currentColor;box-sizing:border-box">CesiumLink scene: shown in the audience window.</div>`
  : html`
  <div
