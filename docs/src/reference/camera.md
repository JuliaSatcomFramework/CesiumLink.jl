...
Only input on the canvas takes the camera, so a button never does.
...
on a satellite. A drag on a camera that rides detaches, but does not dismount: the user steers
around the thing and carries on riding it. The `home` button gets off, and so does any later
viewpoint that flies somewhere else.

A tour rides something through a `Viewpoint`. A listener answering a click rides something through
[`declare_follow`](@ref), which leaves the declared track alone, so one scene does both.
