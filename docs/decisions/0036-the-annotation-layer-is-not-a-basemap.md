...
**The two layers are independent.** Either may be off while the other is on. A reader who wants the
borders without the names, or the names without the borders, gets both answers from two flags.

**They carry no credit and open no origin.** Both files ship inside the viewer, so nothing reaches
