@testitem "a camera track is one declared set, and an empty call clears it" setup=[Furnished] begin
    ...
    try
        ...
        @test length(declared(server, "camera")["track"]) == 2
        ...
        @test length(declared(server, "camera")["track"]) == 1
        declare_camera(server)
        @test declared(server, "camera") == Dict("track" => [])

        # One command per `(module, topic)` is retained, whatever the length of the track.
        @test CesiumLink.retained(server, ("core", "camera")) !== nothing
    finally
        stop_server(server)
    end
end
...
@testitem "the retained frame carries the whole declared viewpoint" setup=[Furnished] begin
    ...
    try
        ...
        v = only(declared(server, "camera")["track"])
        ...
        r = only(declared(server, "camera")["track"])
...
@testitem "a stated label travels and an absent one does not" setup=[Furnished] begin
    ...
    try
        ...
        track = declared(server, "camera")["track"]
...
@testitem "an at index leaves Julia 1-based and arrives 0-based" setup=[Furnished] begin
    ...
    try
        ...
        @test only(declared(server, "camera")["track"])["after"] == 1.5
    finally
        stop_server(server)
    end
end
...
@testitem "a ride goes out on the wire and never displaces the declared track" setup=[Furnished] begin
    ...
    try
        ...
        @test length(declared(server, "camera")["track"]) == 1
        @test only(declared(server, "camera")["track"])["at"] == 3
